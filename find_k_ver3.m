function [k, wrong] = find_k_ver3( u, v, h, l, a, b, eps, print_errors )
wrong = 0;
k = [];
...
if wrong == 0
    ...
    dK0 = - ( (d2ua*d1ua+d2va*d1va)*K0_found - l*(ua*d1va-va*d1ua) ) / (d1ua^2+d1va^2);
    [~, k] = ode45(@(t, y) odefun(t, y), h, [K0_found dK0]);
...
dk0 = - ( (d2ua*d1ua+d2va*d1va)*k0 - l*(ua*d1va-va*d1ua) ) / (d1ua^2+d1va^2);
[~, k_temp] = ode45(@(t, y) odefun(t, y), h, [k0 dk0]);
k_temp = k_temp';

residual = (d1ub^2+d1vb^2)*k_temp(2,end) + (d2ub*d1ub+d2vb*d1vb)*k_temp(1,end) - l*(ub*d1vb-vb*d1ub);
